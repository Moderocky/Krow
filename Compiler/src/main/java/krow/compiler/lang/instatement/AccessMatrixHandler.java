...
public class AccessMatrixHandler implements DefaultHandler {
    ...
    public boolean accepts(String statement, CompileContext context) {
        ...
        if (statement.indexOf(',') == -1 || statement.indexOf(']') == -1) return false;
        if (statement.indexOf(']') < statement.indexOf(',')) return false;
    ...
    public HandleResult handle(String statement, PreClass data, CompileContext context, CompileState state) {
        ...
        assert type != null;
        assert type.isArray();
        assert type.componentType().isArray();
        ...
        return new HandleResult(null, statement.substring(input.length()).trim(), CompileState.MATRIX_ACCESSOR);
    }
    
    @Override
    public String debugName() {
        return "BEGIN_LOAD_MATRIX_ITEM";
    }
}
